...
extension DiscourseClientDataManager: UserImageService {
    func fetchUserImage(avatarURL: String, completion: @escaping (Data) -> Void) {
        remoteDataManager.fetchUserImage(avatarURL: avatarURL, completion: completion)
    }
}
...
extension DiscourseClientDataManager: ConfigDataManager {
    func getConfigFromKeyChain(completion: @escaping (Config) -> ()) {
        localDataManager.getConfigFromKeyChain(completion: completion)
    }
    
    func saveConfigToKeyChain(username: String, apikey: String) {
        localDataManager.saveConfigToKeyChain(username: username, apikey: apikey)
    }
